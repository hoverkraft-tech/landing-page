...
- Partagez vos retours`;
      ...
      mockOpenAIService.generateText.mock.mockImplementation(async (messages) => {
      ...
      const result = await contentGenerator.generateFrenchContent(mockReleasesData, {
      ...
      assert.strictEqual(result.data.repositories[0].releases[0].highlights[0], 'Première version stable');
  ...
  describe('generateEnglishContent', () => {
    it('should fallback when AI summary is empty', async () => {
      ...
      const result = await contentGenerator.generateEnglishContent(mockReleasesData, {
      ...
      assert.deepStrictEqual(result.data.repositories[0].releases[0].highlights, []);
...
- Misc updates`;
    ...
    it('should fallback to basic bullet extraction when sections missing', () => {
      ...
      assert.deepStrictEqual(highlights, ['First improvement', 'Second improvement']);
    });
  });
});
