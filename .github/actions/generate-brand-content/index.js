...
const { assertCore, assertIO, parseJsonInput, requireTrimmedString } = require('./lib/action-inputs');
...
function parseRunInputs({ version, commit, colors, brandMission, logos, mascot, typography, outputDir }) {
  return {
...
async function run({ core, io, ...inputs }) {
  ...
  for (const file of files) {
    ...
    if (Array.isArray(file.payload?.items)) {
      core.info(`✓ Generated ${file.filename} (${file.payload.items.length} items)`);
      continue;
    }
