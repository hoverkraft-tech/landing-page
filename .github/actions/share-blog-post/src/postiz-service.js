...
class PostizService {
  ...
  async createDraftPost({ postId, content, contentByIntegrationType, socialImageUrl, date } = {}) {
  ...
  buildDraftPayload({ postId, content, contentByIntegrationType, socialImageUrl, date }) {
    ...
    if (!normalizedFallbackContent && !normalizedContentByIntegrationType) {
      throw new Error("Either 'content' or 'contentByIntegrationType' must be provided");
    }
      ...
      posts: integrations.map((integration) => ({
        ...(function () {
          const perIntegrationContent =
            normalizedContentByIntegrationType?.[integration.type] ?? normalizedFallbackContent;
  ...
  normalizePostDate(date) {
    ...
    if (Number.isNaN(parsed.getTime())) {
      throw new Error(`Invalid date '${normalized}'. Use an ISO 8601 string like '2025-12-24T10:00:00Z'.`);
    }
  ...
  async createPost(payload) {
    ...
    if (result && typeof result === 'object') {
      ...
      if (
        ...
        throw new Error(`Postiz API error (${statusCode ?? 'unknown'}): ${normalizedMessage}`);
      }
    }
